...
def simulate_ab_fast(textA: str, textB: str) -> Dict[str, Any]:
    ...
    Avoids heavy object initialization and external calls.
    """
    def _score(text: str) -> float:
        ...
        length_score = min(len(words), 80) / 80.0
        hashtag_score = min(t.count("#"), 4) * 0.08
        cta_score = 0.12 if "?" in t else 0.0
        emoji_score = 0.06 if any(ord(ch) > 10000 for ch in t) else 0.0

        # Penalize too-short outputs slightly.
        short_penalty = 0.15 if len(words) < 12 else 0.0
        return max(0.0, min(1.0, 0.45 * length_score + hashtag_score + cta_score + emoji_score - short_penalty))
