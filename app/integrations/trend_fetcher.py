...
try:
    ...
    # Allow backend startup even when spaCy/torch DLLs fail on Windows.
    spacy = None
    STOP_WORDS = set()
...
class TrendFetcher:
    ...
    def __init__(self):
        ...
        if spacy is not None:
            ...
            logging.warning("spaCy unavailable; using regex keyword extraction fallback.")
            self.nlp = None
    ...
    def extract_keywords(self, text: str) -> List[str]:
        if not self.nlp:
            # Basic fallback so features still work without spaCy.
